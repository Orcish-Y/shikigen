...
class NoHttp(importlib.abc.MetaPathFinder):
  def find_spec(self, fullname, path=None, target=None):
    if any(
      fullname == name or fullname.startswith(name + ".") for name in ("fastapi", "app")
    ):
...
from shikigen.runtime import open_runtime  # noqa: E402
...
async def main():
  ...
  with patch("shikigen.runtime.composition.create_lead_agent", new=approval_agent):
    async with open_runtime(config) as runtime:
      thread = await runtime.threads.create_thread()
  ...
  assert not any(
    name == "app" or name.startswith(("app.", "fastapi")) for name in sys.modules
  )
