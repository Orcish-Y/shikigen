...
随后 4A 已补齐新库 schema 约束、消息冲突检查与业务写入返回契约。2026-09-16：4B 已完成离线副本迁移与实际数据副本审计；统一发布接线随后在 4C 完成（见下方验收记录）。
下面仅勾选本次有实现和验证依据的子项。
...
- [x] 列出旧 schema 到新 schema 的字段映射，保留 user_id/title、消息顺序与稳定身份。
- [x] 明确 schema 版本与升级入口，避免依赖“表能创建就算兼容”。
- [x] 在数据库副本中检查是否已经存在同 Thread 多个非终态 Run、消息身份冲突等问题。
- [x] 对历史不一致给出明确处理报告，不静默丢弃，也不直接强加约束后等待失败。
- [x] 明确是否拆分产品库和 checkpoint 库；文件拆分本身不解决跨两者的一致性。
- [x] 验证失败时旧数据仍可读取；切换前停止写入再切换，避免新旧路径同时写两种格式。
- [x] 若旧事件没有记录准确的生命周期时刻，不伪造“历史上真实发生过”的事件；明确迁移合成记录的含义。

### 2026-09-16 4B 验收记录：已完成（副本验证，未切换实际数据库）

实现 [离线迁移入口](../app/persistence/migrate.py)，字段映射、支持版本、问题码、
迁移状态快照语义及停写切换／回滚步骤见 [4B 迁移说明](4b-storage-migration.md)。
`prepare_copy()`／`python -m app.persistence.migrate` 使用 SQLite backup，
默认只审计，`--upgrade` 在审计通过后仅升级新副本；异常回滚产品表重建。
保留 user_id/title、原消息身份／顺序／时间及 checkpoint 表，产品库与 checkpoint 不拆分。
旧 running/interrupted 保留状态，不自动恢复执行；缺失历史事实追加明确标注的迁移观察快照。

已从实际数据库获取副本：1 个 Thread、1 个 completed Run、2 条消息，审计无阻断问题；
升级副本新增 1 条状态快照，原消息与 checkpoint 数据保持。未升级实际来源、未切换配置。
[迁移测试](../tests/test_storage_migration.py) 覆盖旧版有／无事件键、WAL backup、
消息身份／非终态冲突、版本检查、重复执行、事务异常与取消回滚，以及新版业务接口读写。

验证：`.venv/bin/python -m unittest discover -s tests -p 'test_*.py'`，
**166 项通过**（其中新增迁移测试 13 项）；新增 Python 文件通过 Ruff lint／格式检查，
整个 `app/` 通过 ty 类型检查。实际副本额外验证原 Thread、Run 既有字段、消息行、
checkpoint／writes 逐行一致，并通过新版 ChatStore 打开、查询原有两条消息。
测试与副本验证未调用真实模型。
...
- Graph 结束后提交结算、发布事实、关闭流、清理注册表。旧的分支收尾发布已合并；
  harness 的 `run_agent_loop` 仍有独立兼容调用及测试，产品运行入口不使用它。
- 新增 `durable_event`，数据完整保留 `id/thread_id/run_id/seq/event_type/category/`
  ...
  原 `status/error` 保留为已提交状态的兼容投影；HTTP SSE／JSONL 透传完整事实。
