...
`ChatStore` 只在新数据库上安装当前 schema；历史数据库不在支持范围内，部署者需要先删除旧数据库再创建新库。`event_version` 暂不升级，HTTP 继续使用现有 JSONL；事件
...
- 新增 `durable_event`，数据完整保留 `id/thread_id/run_id/seq/event_type/category/`
  ...
  原 `status/error` 保留为已提交状态的现有投影；HTTP SSE／JSONL 透传完整事实。
...
第 1—4 步已验收完成。下一步是第 5 步：消息身份、归属与契约；
