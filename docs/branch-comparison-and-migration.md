...
2026-09-16 4B 更新：已实现并验证离线副本迁移，支持无版本标记的旧 ChatStore
升级到 4A schema v1，保留 Thread 元数据、消息身份／顺序及同文件 checkpoint。
审计报告显式阻止冲突数据升级；缺少生命周期历史时只追加带迁移标记的状态快照。
实际数据库副本审计通过并完成升级演练，来源与配置未改动；统一发布仍属 4C。
字段映射、命令、失败回滚和停写切换流程见 [4B 迁移说明](4b-storage-migration.md)。
...
保留简化终态事件兼容现有传输；此步不提供跨进程可靠投递或自动补发。
